 ...
 * @property {"text"|"string"|"boolean"|"enum"|"date"|"time"|"number"} type  - Phase 9 audit finding: motor.js's real vocabulary is string/boolean/enum/date/time/number ("text" was this contract's own invention, motor.js has never used it). Both "text" and "string" are accepted as synonyms — motor.js injection maps "text"->"string" — rather than rewriting every existing package/dry-run that already says "text".
 * @property {boolean} required
 * @property {string[]} [options]
 * @property {boolean} [autofillable]
 * @property {string} [helpText]        - Phase 9 Del 3/4: shown by the Generic Schema Renderer, never hardcoded in React
 *
 ...
 * @property {"pre_day"|"drift"|"running"|"conversion"} [origin]  - Phase 9 audit finding: motor.js routes schema definitions into 4 separate buckets (PRE_DAY_SCHEMAS/DRIFT_SCHEMAS/RUNNING_SCHEMAS/CONVERSION_SCHEMAS) keyed by this; defaults to "running" if omitted
 * @property {string[]} [triggers]      - Phase 9 audit finding: free-text keywords motor.js used to auto-detect "create this schema" (e.g. sja_preday's ["sja","sikker jobb"]) were hardcoded per-org in motor.js; now organization-suppliable
