...
export class CorrectionMemoryStore {
   ...
   * @param {{category: import('./types.mjs').CorrectionCategory, originalValue: string, correctedValue: string, userId: string, organizationId: string}} input
   * @param {Date} [now]
   * @returns {import('./types.mjs').CorrectionEntry}
   */
  recordCorrection(input, now = new Date()) {
    const { category, originalValue, correctedValue, userId, organizationId } = input;
    ...
    if (existing && existing.correctedValue === correctedValue) {
      ...
      existing.confidenceBonus = bonusForCount(existing.count);
    ...
    const entry = {
      ...
      category, originalValue, correctedValue, userId, organizationId,
      count: 1, createdAt: now.toISOString(), lastUsedAt: now.toISOString(),
      confidenceBonus: bonusForCount(1), active: true,
    };
