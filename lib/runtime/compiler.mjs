...
export function compileRuntime(input, publishContext) {
  ...
  const kg = input.knowledgeGraph || { activities: [], machineTypes: [], orderTypes: [] };
  const machineTypeIds = new Set(kg.machineTypes.map((m) => m.id));
  const activityIds = new Set(kg.activities.map((a) => a.id));
  for (const mt of kg.machineTypes) {
