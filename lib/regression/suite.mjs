...
export const REGRESSION_CASES = [
  ...
  {
    id: "phaseA_partial_knowledge_graph_does_not_crash_compile",
    description: "Validation Sprint Del 6: a minimal, legitimate organization package that omits machineTypes/orderTypes from knowledge_graph.json (e.g. no machines to track) must compile cleanly or fail with a real validation error — not crash with a raw 'Cannot read properties of undefined (reading map)' TypeError, found via a real corrupt/empty-Runtime injection test.",
    run: () => {
      const input = {
        organizationContext: { organizationId: "kg_partial_test", name: "KG Partial Test", orders: [], machines: [], vehicles: [], wageCodes: [], procedures: [], externalLinks: [], sjaDefaults: null },
        schemas: [],
        rules: [],
        capabilityProviders: [],
        capabilityBindings: [],
        aliases: [],
        knowledgeGraph: { activities: [] }, // deliberately missing machineTypes and orderTypes
        preferredCandidates: [],
        rankingWeights: {},
        capabilityKindExpectations: {},
        promptLabels: {},
        extractionPatterns: {},
        extractionVocabularies: {},
      };
      const result = compileRuntime(input, { runtimeVersion: 1 });
      return result.valid === true && result.runtime !== null;
    },
  },
  {
    id: "sprint1_runtime_history_pruning_never_removes_active_or_floor",
    description: "Execution Sprint 1 Oppgave 3: pruneHistory() must remove only old, non-active versions beyond the keepVersions floor, and must NEVER remove the currently active version regardless of its age — verified across many simulated versions with deliberately old timestamps, including an old version reactivated via rollback.",
    run: () => {
      const store = new RuntimeStore();
      const orgId = "prune_test_org";
      const oldDate = new Date(Date.now() - 400 * 24 * 60 * 60 * 1000).toISOString(); // 400 days old — well beyond any retention window
      // Publish 30 versions, then backdate all but the last 5 to simulate a long-running org.
      for (let v = 1; v <= 30; v++) {
        store.publish({ organizationId: orgId, runtimeVersion: v, checksum: "cs_" + v, schemas: [] }, "test");
      }
      const manifests = store.manifestsByOrg.get(orgId);
      for (let i = 0; i < manifests.length - 5; i++) manifests[i].publishedAt = oldDate;

      // Roll back to an OLD version (v3) so it becomes "active" despite its old publishedAt timestamp.
      store.rollback(orgId, 3);

      const { prunedCount } = store.pruneHistory(orgId, { keepVersions: 10, keepDays: 180 });
      const remaining = store.history(orgId);
      const activeStillPresent = remaining.some((m) => m.runtimeVersion === 3 && m.status === "active");
      const activeRuntimeStillFetchable = store.getActive(orgId) !== null && store.getActive(orgId).runtimeVersion === 3;
      const floorRespected = remaining.length >= 10;

      return prunedCount > 0 && activeStillPresent && activeRuntimeStillFetchable && floorRespected;
    },
  },
];
