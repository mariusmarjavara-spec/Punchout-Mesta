...
export function bootMotor(runtime) {
  ...
  const windowStub = {
    // Post-pilot baseline correction: this used to set
    // `hovedordre: runtime.runtimeConfig.hoofdordre`, conflating the
    // organization's primary ACTIVE WORK ORDER with the reserved main-timesheet
    // bucket key. Harmless only because motor.js's normalizeConfig() happened to
    // ignore `hovedordre` entirely at the time (the very defect this mission
    // fixed) — the moment that field started working, this line collided the
    // scenario's own work draft with the main-time bucket and this suite's
    // descriptionIntegrity stage caught it. The two are separate fields; see
    // normalizeConfig() in public/motor.js.
    PUNCHOUT_CONFIG: runtime
      ? { lonnskoder: runtime.runtimeConfig.lonnskoder, kjoretoy: runtime.runtimeConfig.kjoretoy, sjaDefaults: {}, externalLinks: runtime.runtimeConfig.externalLinks, hovedordre: "HOVED", hoofdordre: runtime.runtimeConfig.hoofdordre }
      : { lonnskoder: [], kjoretoy: [], sjaDefaults: {}, externalLinks: [], hovedordre: "HOVED", hoofdordre: "" },
