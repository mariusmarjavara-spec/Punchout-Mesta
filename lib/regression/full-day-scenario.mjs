 ...
 */
export async function runFullDayScenario(orgDir) {
  let stage = "load";
  try {
    ...
    stage = "export";
    const envelope = buildExportEnvelope({ ...snapshot.dayLog, status: "LOCKED" }, { organizationId: runtime.organizationId, userId: "test_user", deviceId: "test_device", appVersion: "0.9.0" });

    stage = "adapter";
    const adapterResult = await runAdapter(LandaxAdapter, envelope, () => {});

    return {
      ok: allResolved && locked && adapterResult.ok,
      stage: "complete",
      organizationId: runtime.organizationId,
      detail: {
        rankingCheck,
