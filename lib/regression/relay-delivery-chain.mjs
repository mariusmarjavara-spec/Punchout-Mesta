...
export async function runRelayDeliveryChainCheck() {
  ...
  let server = await startNextDevServer({ cwd: process.cwd(), port: PORT, env, timeoutMs: 90000 });

  try {
    check("relay_chain_server_starts", server.readiness.ok, server.readiness.ok
      ? null
      : { message: "dev server did not respond on /api/health", diagnostics: server.diagnostics });
    ...
    server = await startNextDevServer({ cwd: process.cwd(), port: PORT, env, timeoutMs: 90000 });
    check("relay_chain_server_restarts", server.readiness.ok, server.readiness.ok
      ? null
      : { message: "server did not come back up", diagnostics: server.diagnostics });

    if (server.readiness.ok) {
