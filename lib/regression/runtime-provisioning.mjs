...
export async function runRuntimeProvisioningCheck() {
  ...
  try {
    check("runtime_provisioning_server_starts", server.readiness.ok, server.readiness.ok
      ? null
    ...
    // Deliberately BEFORE any publish/provision call below: a real severe
    // bug (found via real-browser testing, reproduced against the actual
    // production Docker build, fixed the same session) only manifested when
    // an unprovisioned request hit "/" before any provisioned one did --
    // whichever variant rendered first got served to everyone thereafter,
    // regardless of that later request's own cookie. Testing this order
    // first is the whole point of this check, not incidental.
    const htmlColdStart = await fetchRootAs(null);
    check("runtime_provisioning_unprovisioned_first_request_falls_back_safely", htmlColdStart.includes("punchout-config.js") && !htmlColdStart.includes("PUNCHOUT_RUNTIME"));
    ...
    check("runtime_provisioning_unprovisioned_falls_back_after_other_activity", htmlUnprovisioned.includes("punchout-config.js") && !htmlUnprovisioned.includes("PUNCHOUT_RUNTIME"));
