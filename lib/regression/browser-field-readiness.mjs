...
export async function runBrowserFieldReadiness() {
  ...
  const server = await startNextDevServer({ cwd: process.cwd(), port: PORT, env, timeoutMs: 90000 });

  let browser;
  try {
    check("field_readiness_server_starts", server.readiness.ok, server.readiness.ok
      ? null
      : { message: "dev server did not respond", diagnostics: server.diagnostics });
    ...
    // ── GUIDED SJA, on a phone, through the real worker path ─────────────
    //
    // Everything below reaches the flow the way a worker does: the pre-day
    // screen, the SJA card, "Fyll ut". Engine and component tests prove the
    // model; only this proves the model is REACHABLE.
    //
    // The day is seeded with a natural sentence first, so the flow has real
    // context to reuse rather than a fixture handed to it.
    await page.evaluate(() => {
    ...
    const sjaCard = page.locator("text=SJA").first();
    check("guided_sja_card_present_on_preday", (await sjaCard.count()) > 0);

    const fillButtons = page.getByRole("button", { name: "Fyll ut" });
    check("guided_sja_has_a_fill_action", (await fillButtons.count()) > 0);
    await fillButtons.first().tap();
    await page.waitForTimeout(700);

    check("guided_sja_no_horizontal_scroll", await noHorizontalScroll(page));

    // One prompt dominates — the later prompts must not be co-present.
    const firstPrompt = await page.evaluate(() => document.body.innerText);
    check(
      "guided_sja_shows_one_prompt_not_a_form",
      firstPrompt.includes("Hva skal du gjøre i dag?") &&
        !firstPrompt.includes("Hva kan konsekvensen bli?"),
      { excerpt: firstPrompt.slice(0, 240) },
    );

    // The context typed a moment ago comes back as something to agree with.
    check(
      "guided_sja_reuses_what_the_worker_already_said",
      firstPrompt.includes("Punchout oppfattet") && firstPrompt.includes("Grøfterensk"),
      { excerpt: firstPrompt.slice(0, 240) },
    );

    const stemmer = page.getByRole("button", { name: "Stemmer" });
    const stemmerBox = await stemmer.first().boundingBox();
    check(
      "guided_sja_accept_touch_target_adequate",
      !!stemmerBox && stemmerBox.height >= MIN_TOUCH_TARGET,
      { height: stemmerBox?.height },
    );

    await stemmer.first().tap();          // oppgave
    await page.waitForTimeout(400);
    const stedText = await page.evaluate(() => document.body.innerText);
    check(
      "guided_sja_prefills_the_place_from_the_entry",
      stedText.includes("Hvor skal arbeidet utføres?") && stedText.includes("RV92 km 14"),
      { excerpt: stedText.slice(0, 240) },
    );
    await page.getByRole("button", { name: "Stemmer" }).first().tap();  // sted
    await page.waitForTimeout(400);

    // The judgement step: no accept shortcut, suggestions unselected.
    const riskText = await page.evaluate(() => document.body.innerText);
    const acceptCount = await page.getByRole("button", { name: "Stemmer" }).count();
    check(
      "guided_sja_judgement_step_offers_no_accept_shortcut",
      riskText.includes("Hva kan gå galt?") && acceptCount === 0,
      { acceptCount, excerpt: riskText.slice(0, 200) },
    );
    const preSelected = await page.locator('[aria-pressed="true"]').count();
    check("guided_sja_no_risk_is_preselected", preSelected === 0, { preSelected });

    // Accept two proposals, then add one of the worker's own.
    const proposals = page.locator('[aria-pressed="false"]');
    const proposalCount = await proposals.count();
    if (proposalCount > 0) await proposals.first().tap();
    await page.waitForTimeout(200);
    check(
      "guided_sja_risk_can_be_selected_and_deselected",
      (await page.locator('[aria-pressed="true"]').count()) === 1,
    );
    await page.getByRole("button", { name: "Neste" }).first().tap();
    await page.waitForTimeout(400);

    // Remaining judgement + factual steps, answered as a worker would.
    for (const answer of ["Personskade ved påkjørsel", "TMA, arbeidsvarsling og samband"]) {
      await page.getByPlaceholder("Skriv eller bruk tale").fill(answer);
      await page.getByRole("button", { name: "Neste" }).first().tap();
      await page.waitForTimeout(400);
    }

    // The work-warning plan, reused from the same sentence.
    const planText = await page.evaluate(() => document.body.innerText);
    check(
      "guided_sja_reuses_the_work_warning_plan",
      planText.includes("Er arbeidsvarslingen på plass?") && planText.includes("24-184"),
      { excerpt: planText.slice(0, 240) },
    );
    await page.getByRole("button", { name: "Stemmer" }).first().tap();
    await page.waitForTimeout(400);

    await page.getByPlaceholder("Skriv eller bruk tale").fill("3 personer");
    await page.getByRole("button", { name: "Neste" }).first().tap();
    await page.waitForTimeout(600);

    // ── Interruption and resume, mid-flow ────────────────────────────────
    const beforeReload = await page.evaluate(
      () => window.Motor.getGuidedFormState("sja")?.stepIndex ?? -1,
    );
    ...
    check(
      "guided_sja_progress_survives_a_refresh",
      beforeReload > 0 && afterReload === beforeReload,
      { beforeReload, afterReload },
    );

    // ── Review and explicit confirmation ─────────────────────────────────
    //
    // No re-entry needed, and that is the finding: the reload lands straight
    // back INSIDE the guided form, because both the overlay and the step state
    // are domain state. The worker does not have to find their way back in.
    const reviewText = await page.evaluate(() => document.body.innerText);
    check(
      "guided_sja_reload_returns_into_the_form_not_to_the_list",
      reviewText.includes("Bekreft SJA") && !reviewText.includes("Fyll ut"),
      { excerpt: reviewText.slice(0, 200) },
    );
    check(
      "guided_sja_review_separates_authored_from_derived",
      reviewText.includes("Punchout fylte ut") && reviewText.includes("Registrert automatisk"),
      { excerpt: reviewText.slice(0, 400) },
    );
    check("guided_sja_review_no_horizontal_scroll", await noHorizontalScroll(page));

    const confirm = page.getByRole("button", { name: /Bekreft SJA/ });
    check("guided_sja_confirmation_is_explicit", (await confirm.count()) > 0);
    await confirm.first().tap();
    await page.waitForTimeout(800);

    // ── What actually landed in the schema ───────────────────────────────
    const stored = await page.evaluate(() => {
      const s = (window.Motor.getSnapshot().dayLog?.schemas ?? []).find(
        (x) => x.type === "sja_preday",
      );
      return s ? { fields: s.fields, provenance: s.fieldProvenance } : null;
    });
    check(
      "guided_sja_writes_reused_facts_into_the_schema",
      stored?.fields?.sted?.includes("RV92") &&
        stored?.fields?.arbeidsvarslingsplan === "24-184",
      { fields: stored?.fields },
    );
    check(
      "guided_sja_records_where_each_value_came_from",
      stored?.provenance?.sted?.origin === "INFERRED_CONFIRMED" &&
        stored?.provenance?.konsekvens?.origin === "WORKER",
      { provenance: stored?.provenance },
    );
    check(
      "guided_sja_never_auto_filled_a_judgement_field",
      stored?.provenance?.risiko?.origin !== "INFERRED_CONFIRMED" &&
        stored?.provenance?.risiko?.origin !== "SYSTEM",
      { provenance: stored?.provenance },
    );
