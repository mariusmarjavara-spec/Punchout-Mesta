...
function bootMotor(opts) {
  ...
  /* A frozen clock, when a case needs one.
   *
   * motor.js stamps endDay() from the wall clock, so any assertion about the
   * span between the day's start and its end silently depends on what time the
   * suite happens to run. That is not hypothetical: the main-time suggestion
   * case passed all evening and began failing after midnight, because
   * `endTime - 3.5h` fell before the 07:00 start and the suggestion became
   * null. A regression gate that reports red for reasons unrelated to the code
   * teaches everyone to re-run it.
   *
   * Only `new Date()` and `Date.now()` are pinned. Explicit arguments still
   * construct the date asked for, so existing cases that build specific dates
   * are unaffected. */
  const FixedDate = options.now
    ? class extends Date {
        constructor(...args) {
          if (args.length === 0) super(options.now);
          else super(...args);
        }
        static now() {
  ...
  const sandbox = {
    ...
    setTimeout, clearTimeout, setInterval, clearInterval, Date: FixedDate, JSON, Math, Object, Array, String, Number, Boolean, RegExp, Error,
  };
...
export const MOTOR_CHARACTERIZATION_CASES = [
  ...
  },
  {
    ...
    run: () => {
      // Pinned to mid-afternoon: this assertion measures the span from the
      // day's start to endDay()'s wall-clock stamp, so without a fixed clock it
      // passes in the afternoon and fails in the morning.
      const { Motor } = bootIntoDrift({ now: "2026-01-15T15:00:00" });
      const text = "204481-0149 fra 07:30 til 11:00 asfaltarbeid";
      Motor.confirmStructuredEntry(text, "arbeid", Motor.parseEntry(text));
      Motor.endDay();
