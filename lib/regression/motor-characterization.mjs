...
export const MOTOR_CHARACTERIZATION_CASES = [
  ...
  {
    id: "char_force_skipped_pre_day_schema_does_NOT_block_lock_today",
    description:
      "PINS A CONTRADICTION, DOES NOT FIX IT. forceStartDay()'s own comment states 'force_skipped items MUST be resolved in Håndrens — they block lockDay'. They do not: getUnresolvedItems() drops every pre_day schema before its status is examined, so the 'force_skipped' branch in that status check is unreachable. Dormant in the pilot (nothing can populate requiredSchemas today), which is why this is characterized rather than changed. See docs/POST_PILOT_ARCHITECTURE.md, debt item 3.",
    run: () => {
      const { Motor } = bootMotor();
      Motor.startDay();
      Motor.forceStartDay();
      Motor.endDay();
      return Motor.getSnapshot().dayLog.phase === "ending"
        && Motor.getUnresolvedItems().every((i) => i.kind !== "schema");
    },
  },
  ...
  {
    id: "char_main_time_confirm_is_UNREACHABLE_through_the_react_bridge_today",
    description:
      "PINS A CONTRADICTION, DOES NOT FIX IT. The main timesheet can only ever be DISCARDED in the shipped app. Confirming it requires a lønnskode line; grovutfyllMainDraft() deliberately adds none; confirmStructuredEntry() only ever writes lønnskoder onto a parsed ORDER draft, never the main one; and teAddLonnskode()/addLonnskode() are vanilla-DOM functions absent from the window.Motor bridge. Since buildExportPacket() exports only confirmed drafts, no pilot day has ever exported a main-time line. Characterized, not fixed: closing it means building a lønnskode editor, which is feature work. See docs/POST_PILOT_ARCHITECTURE.md, debt item 1.",
    run: () => {
      const { Motor } = bootIntoDrift();
      Motor.endDay();
      const bridgeCanAddLonnskoder = ["teAddLonnskode", "addLonnskode", "setDraftLonnskoder", "addMainLonnskode"]
        .some((fn) => typeof Motor[fn] === "function");
      Motor.openMainTimeEntryOverlay();
      Motor.confirmMainTimeEntry();
      const log = Motor.getSnapshot().dayLog;
      return bridgeCanAddLonnskoder === false
        && log.drafts["HOVED"].lonnskoder.length === 0
        && log.mainTimeHandled === false;
    },
  },
  {
  ...
  {
    id: "char_failed_write_surfaces_a_save_storage_error_but_keeps_the_day_in_memory",
    description:
      "PINS ACTUAL BEHAVIOUR. When localStorage.setItem throws (quota exceeded, Safari private mode), saveCurrentDay() catches it and publishes storageError type 'save' — but the in-memory day keeps advancing, now AHEAD of what is persisted, with no retry and no write-ahead buffer. A refresh at this point loses everything since the last successful write. Characterized, not fixed; ranked as debt item 2.",
    run: () => {
      const ctx = bootIntoDrift();
