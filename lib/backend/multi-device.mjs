...
for (const id of deviceIds) {
  const res = await fetch(BASE_URL + "/api/devices/register", { method: "POST", headers: { "Content-Type": "application/json", ...authHeader }, body: JSON.stringify({ deviceId: id, organizationId: orgSlug }) });
...
sandboxes.forEach((sandbox, i) => {
  ...
  // Operation Punchout Soft Launch, Phase B: this used to (ab)use hovedordre
  // (the main ORDER number field) as a stand-in unique-per-device tag, back
  // when emitTelemetry itself made the same field-confusion mistake. Now that
  // emitTelemetry tags events with the real ADMIN_CONFIG.organizationId, this
  // sets that field directly instead — same test intent (three distinguishable
  // per-device identities to prove telemetry isolation), correct field.
  sandbox.ADMIN_CONFIG.organizationId = "multidev_org_" + ["A", "B", "C"][i] + "_" + Date.now();
...
const finalExports = await (await fetch(BASE_URL + "/api/export", { headers: authHeader })).json();
...
for (let i = 0; i < 3; i++) {
  const orgTelemetry = await (await fetch(BASE_URL + "/api/telemetry?org=" + sandboxes[i].ADMIN_CONFIG.organizationId, { headers: authHeader })).json();
  record("Telemetri for enhet " + ["A", "B", "C"][i] + " er isolert og komplett (ingen kryssforurensning fra de andre to enhetene)", orgTelemetry.count > 0 && orgTelemetry.events.every((e) => e.organizationId === sandboxes[i].ADMIN_CONFIG.organizationId));
}
