 ...
 * @type {Map<string, {secret: string, registeredAt: string, registeredBy: string, status: "active"|"disabled"}>}
 */
...
export function registerDevice(deviceId, registeredBy, secret) {
  ...
  deviceRegistry.set(deviceId, { secret: finalSecret, registeredAt: new Date().toISOString(), registeredBy, status: "active" });
...
export function listDevices() {
  return [...deviceRegistry.entries()].map(([deviceId, v]) => ({ deviceId, registeredAt: v.registeredAt, registeredBy: v.registeredBy, status: v.status }));
}
