 ...
 *
 * Deterministic retention, with age as the hard upper bound.
 *
 * An entry older than `keepDays` is removed, always. `keepMinimum` may only
 * protect entries already inside the window; it can no longer keep older data
 * alive.
 *
 * This used to work the other way round. The most recent `keepMinimum` entries
 * were kept regardless of age, and a log shorter than `keepMinimum` returned
 * early without pruning anything — so `{keepDays: 90, keepMinimum: 200}` meant
 * ninety days in a busy deployment and forever in a quiet one. Every low-volume
 * pilot was in the second case, holding deviceId-linked records indefinitely
 * while the configuration described itself as 90-day retention. The original
 * intent was benign (a paused pilot should not prune down to nothing), but a
 * stated retention policy cannot survive that effect.
 *
 * Founder decision 2026-08-19: age wins. `keepMinimum` stays in the signature
 * because callers pass it, and it still bounds nothing away inside the window —
 * it simply cannot extend past `keepDays` any more.
 *
 * Deliberately not applied to RuntimeStore.pruneHistory. That floor protects the
 * active version and any version rollback might be asked to reactivate, which is
 * a correctness constraint on operational config rather than retention of
 * records.
 *
 * Mutates the array in place (both logs are `export const`, not reassignable) —
 * the same array reference callers already hold stays valid.
 * @param {any[]} log
 * @param {string} dateField
 * @param {{keepDays: number, keepMinimum?: number}} opts
 */
export function pruneLogInPlace(log, dateField, { keepDays }) {
  const cutoffMs = Date.now() - keepDays * 24 * 60 * 60 * 1000;
  const kept = log.filter((entry) => {
    const at = new Date(entry[dateField]).getTime();
    // An entry whose timestamp cannot be read cannot be shown to be inside the
    // window, and retention is the wrong place to give something the benefit of
    // the doubt.
...
/**
 * The next runtime version for an organization, from its history alone.
 *
 * PERSISTENCE_CONTRACT.md C3 in one function. Extracted from
 * `compileFromPackage` so the contract's known gap has a name and can be
 * tested directly: this READ and the `publish` WRITE that follows it are not
 * atomic, so two callers observing the same history necessarily compute the
 * same number. C2's duplicate-version rejection catches that after the fact
 * rather than preventing it, which the contract is explicit about being the
 * weaker guarantee.
 *
 * Pure and history-scoped on purpose. When a store arrives that can allocate
 * atomically — a sequence, a unique constraint, a compare-and-swap — this is
 * the single function it replaces, and lib/regression/persistence-contract-cases.mjs
 * is the file that must be updated deliberately when it does.
 *
 * @param {{runtimeVersion: number}[]} history
 * @returns {number}
 */
export function allocateNextRuntimeVersion(history) {
  return history.length ? Math.max(...history.map((m) => m.runtimeVersion)) + 1 : 1;
}
...
export function compileFromPackage(organizationSlug) {
  ...
  const nextVersion = allocateNextRuntimeVersion(history);
