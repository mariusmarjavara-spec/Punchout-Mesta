...
async function registerDevice(deviceId) {
  const res = await fetch(BASE_URL + "/api/devices/register", { method: "POST", headers: { "Content-Type": "application/json", "Authorization": "Bearer " + ADMIN_TOKEN }, body: JSON.stringify({ deviceId, organizationId: "mesta" }) });
...
section("Scenario 3: telemetri offline, flush ved reconnect");
{
  ...
  const { sandbox, netState } = bootMotor(deviceId);
  sandbox.ADMIN_CONFIG.userId = "u1";
  // Operation Punchout Soft Launch, Phase B: was hovedordre (main order
  // number field) as a unique-tag stand-in, matching emitTelemetry's own
  // former mistake — now that emitTelemetry tags with the real
  // organizationId, this scenario's isolation tag lives there too.
  sandbox.ADMIN_CONFIG.organizationId = "offline_telemetry_org_" + Date.now();
  sandbox.ADMIN_CONFIG.telemetryEndpoint = BASE_URL + "/api/telemetry";
  const Motor = sandbox.window.Motor;
  netState.up = false;
  Motor.startDay();
  ...
  const backendCheck = await (await fetch(BASE_URL + "/api/telemetry?org=" + sandbox.ADMIN_CONFIG.organizationId, { headers: { "Authorization": "Bearer " + ADMIN_TOKEN } })).json();
