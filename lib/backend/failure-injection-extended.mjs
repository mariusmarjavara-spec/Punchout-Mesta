...
section("Korrupt Runtime-pakke");
{
  ...
  try {
    ...
    const historyAfter = await (await fetch(BASE_URL + "/api/runtime/history?org=" + orgSlug)).json();
...
section("Halvveis publisering — dry-run feiler, ingen manifest skal opprettes");
{
  ...
  try {
    ...
    const history = await (await fetch(BASE_URL + "/api/runtime/history?org=" + orgSlug)).json();
...
section("Halvveis eksport — klient avbryter midt i forespørselen");
{
  ...
  await p;
  await tick(200);
  const log = await (await fetch(BASE_URL + "/api/export")).json();
  ...
  const followUp = await fetch(BASE_URL + "/api/export");
