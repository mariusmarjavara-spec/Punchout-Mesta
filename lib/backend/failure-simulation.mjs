...
// 4. Export API unavailable — point motor.js's real syncExports() at an unreachable port
{
  ...
  const regRes = await fetch(BASE_URL + "/api/devices/register", { method: "POST", headers: { "Content-Type": "application/json", ...auth(ADMIN_TOKEN) }, body: JSON.stringify({ deviceId }) });
...
// separately by cases above; this isolates the concurrency question cleanly, as intended).
{
  ...
  await fetch(BASE_URL + "/api/devices/register", { method: "POST", headers: { "Content-Type": "application/json", ...auth(ADMIN_TOKEN) }, body: JSON.stringify({ deviceId }) });
