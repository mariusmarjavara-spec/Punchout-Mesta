...
// Operation Punchout Soft Launch, Phase B: organizationId is now a real,
// separately-tracked field (see motor.js's ADMIN_CONFIG wiring fix and
// emitTelemetry's fix, same session) — previously telemetry was tagged with
// ADMIN_CONFIG.hovedordre (the main ORDER number) as a stand-in, which this
// script's own verification query below used to match. Using the real,
// already-known organizationId (from the publish step above) end-to-end now.
sandbox.ADMIN_CONFIG.organizationId = pub1.manifest.organizationId;
...
await tick(400);
const telemetryCheck = await (await fetch(BASE_URL + "/api/telemetry?org=" + sandbox.ADMIN_CONFIG.organizationId, { headers: authHeader })).json();
