 ...
 * @param {Date} [now]
 * @param {Record<string,string>} [promptLabels]  - Runtime.promptLabels (lib/runtime/types.mjs), keyed by action target
 * @returns {import('./types.mjs').PromptQueueItem[]}
 */
export function buildPromptQueue(facts, rules, capabilityBindings = [], now = new Date(), promptLabels = {}) {
  ...
  for (const factKey of uniqueKeys) {
    ...
    for (const rule of matched) {
      if (rule.action.target === "$factValue") {
        // Per-instance, not per-key: the flat context collapses multiple
        // facts sharing a key to one value, which would silently drop
        // e.g. a second required schema from the same machine. Iterate
        // every actual fact with this key instead.
        for (const f of facts.filter((x) => x.key === factKey)) {
          required.push({ ruleId: rule.id, factKey, action: { ...rule.action, target: String(f.value) }, priority: rule.priority });
        }
      } else {
        required.push({ ruleId: rule.id, factKey, action: rule.action, priority: rule.priority });
      }
    }
  }
  ...
  for (const req of required) {
    ...
    if (existing) {
      ...
      byId.set(id, {
        ...
        label: promptLabels[req.action.target] || (req.action.type + ":" + req.action.target),
      });
    }
  }
