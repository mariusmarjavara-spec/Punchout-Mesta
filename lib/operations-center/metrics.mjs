...
/**
 * po-crash-telemetry-aggregation: motor.js's global error/unhandledrejection
 * handlers have captured real crashes as ClientError telemetry since the
 * post-sprint-3-strategic-review.md fix — but nothing read or aggregated
 * them, "a real, named blind spot for the least digitally-confident users,
 * who are least likely to self-report." This is that read side.
 *
 * Groups by message (the closest thing to a crash signature this event
 * carries — no error class/fingerprint field exists) so the same
 * recurring bug across many devices/sessions shows as one entry with a
 * real count, not N indistinguishable rows.
 * @param {import('../telemetry/types.mjs').TelemetryEvent[]} events
 * @param {number} [recentLimit] how many of the most recent raw events to include verbatim
 */
export function crashSummary(events, recentLimit = 20) {
  const crashes = events.filter((e) => e.type === "ClientError");
  const byMessage = {};
  for (const e of crashes) {
    const message = e.data?.message || "(no message)";
    byMessage[message] = byMessage[message] || { message, count: 0, organizationIds: new Set(), lastSeenAt: null };
    const entry = byMessage[message];
    entry.count++;
    entry.organizationIds.add(e.organizationId);
    if (!entry.lastSeenAt || e.occurredAt > entry.lastSeenAt) entry.lastSeenAt = e.occurredAt;
  }
  const bySignature = Object.values(byMessage)
    .map((entry) => ({ ...entry, organizationIds: [...entry.organizationIds] }))
  ...
  const recent = [...crashes]
    ...
    .map((e) => ({
      ...
      message: e.data?.message ?? null,
      source: e.data?.source ?? null,
      line: e.data?.line ?? null,
      stack: e.data?.stack ?? null,
    }));

  return { totalCount: crashes.length, bySignature, recent };
}
